...
//#include "../ui/ui.h"
#include "../gfx/LGFX_ESP32S3_Elecrow70.hpp"

#include "Free_Fonts.h"     // Free fonts
#include "Stromfluss.h"     // picture
...
// drawing examples, see https://lovyangfx.readthedocs.io/en/latest/02_using.html
void drawExamples()
{
  ...
  lcd.setTextSize(1);
  //lcd.setFreeFont(FSS9);  // Sans Serif 9 pt
  lcd.setFont(FSS9);  // Sans Serif 9 pt
  bgndColor = TFT_WHITE; // TFT_YELLOW;
  fgndColor = TFT_BLACK; 
  lcd.setTextColor(fgndColor, bgndColor);

  lcd.setCursor(0,0);
  lcd.write("Test start 0,0");
  lcd.setCursor(screenWidth/2,screenHeight/2);
  sprintf(printstring, "Test start %d, %d", screenWidth/2, screenHeight/2);
  lcd.write(printstring);
  delay(1000);

  lcd.fillScreen(TFT_YELLOW);
  lcd.setSwapBytes(true); // correct for endian-ness of the data 
  lcd.pushImage(100, 100, 320, 240, image_data_Image); // works and fast
  delay(1000);
...
/*Read the touchpad*/
// returns the x and y position (calibrated to screen resolution) if touched and 0,0 if touch released
...
void touchpadTest()
{
  ...
  touchpadRead(&xTouch, &yTouch);
  lcd.setCursor(100,100);
  // lcd.setCursor(screenWidth/2,screenHeight/2);
  if(xTouch > 0 && yTouch >0)
    sprintf(printstring, "Touch read %d, %d   ", xTouch, yTouch);
  else
    sprintf(printstring, "Touch released            ");
  lcd.write(printstring);
}  
...
void gui_start(){
  // ----------- GFX basic settings -------------
  lcd.init();
  lcd.setColorDepth(16); // 16 bit or 24 bit color
  lcd.setRotation(0); // 1 = portrait
